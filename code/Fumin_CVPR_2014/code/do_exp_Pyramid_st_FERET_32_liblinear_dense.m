...
%%%%%%%%%%%% Liblinear SVM %%%%%%%%%%%%%%%%%%%%%
 ...
 addpath /home/pli/2_Packages/liblinear-1.5-dense/matlab
 model = {train(double(trainY),trainXCs,'-s 3 -c 1')};
fprintf('Start testing SVM\n');
 [~, accs] = predict(double(testY),testXCs,model);
 acc = accs(1);
 save(['./res_svm/acc_' dataset '_mycoding_' alg '_' encoder], 'acc');
 fprintf('Test accuracy %f%%\n', acc);
p = profile('info');
profsave(p,'profile_results_liblinear_dense')
