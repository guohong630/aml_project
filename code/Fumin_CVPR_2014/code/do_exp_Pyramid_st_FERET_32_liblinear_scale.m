...
% Scale data
minimums = min(trainXC, [], 1);
ranges = max(trainXC, [], 1) - minimums;

trainXCs = (trainXC - repmat(minimums, size(trainXC, 1), 1)) ./ repmat(ranges, size(trainXC, 1), 1);
testXCs = (testXC - repmat(minimums, size(testXC, 1), 1)) ./ repmat(ranges, size(testXC, 1), 1);

trainXCs = [trainXCs, ones(size(trainXCs,1),1)]; % intercept term
testXCs = [testXCs, ones(size(testXCs,1),1)];

% trainXCs = (trainXC - repmat(min(trainXC,[],1),size(trainXC,1),1))*spdiags(1./(max(trainXC,[],1)-min(trainXC,[],1))',0,size(trainXC,2),size(trainXC,2));
% testXCs = (testXC - repmat(min(testXC,[],1),size(testXC,1),1))*spdiags(1./(max(testXC,[],1)-min(testXC,[],1))',0,size(testXC,2),size(testXC,2));
...
%%%%%%%%%%%% Liblinear SVM %%%%%%%%%%%%%%%%%%%%%
 ...
 model = {train(double(trainY),sparse(trainXCs),'-q -s 3 -c 1')};
 fprintf('Start testing SVM\n');
...
p = profile('info');
profsave(p,'profile_results_liblinear_scale')
